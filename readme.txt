=== Dynamic Blog Protector - Protect Your Content ===
Contributors: Chetan

Donate link: http://111waystomakemoney.com/donate/
Plugin URI: http://111waystomakemoney.com/dynamic-blog-protector/
Author URI: http://111waystomakemoney.com/
Tags: post, posts, copy, protect, right, click, disable, copyprotect, Google, SEO, page, plugin, access, content, javascript, security, protector, widget, admin, sidebar, comments, images, google, links, twitter, rss
Requires at least: 2
Tested up to: 3.0
Stable tag: 1.3

Protect your valuable blog content as well as images from getting copied.

== Description ==
For detailed description go to plugins page at [Dynamic Blog Protector](http://111waystomakemoney.com/dynamic-blog-protector/)

Protect your valuable blog content(Pages & Posts) from getting copied.

= Features =
* **Disable right click** on your blog 
* **Disable selection of text** on your blog

You must enable the features from options page to protect your blog.

Please dont forget to give Ratings to Plugin.

If you like Dynamic Blog Protector, Please Consider A Donation.

(http://111waystomakemoney.com/donate/).
Thank You.

Warm Regards,
Chetan
(http://111waystomakemoney.com/).
For detailed and description go to plugins page at [Dynamic Blog Protector](http://111waystomakemoney.com/dynamic-blog-protector/)

== Installation ==
For detailed Instructions go to plugins page at [Dynamic Blog Protector](http://111waystomakemoney.com/dynamic-blog-protector/)
1. Upload `blog-protector.php` to the `/wp-content/plugins/` directory
2. Activate the plugin through the 'Plugins' menu in WordPress
3. Go to **Settings** and then **Dynamic Blog Protector** to configure and protect your blog.
For detailed Instructions go to plugins page at [Dynamic Blog Protector](http://111waystomakemoney.com/dynamic-blog-protector/)

== Frequently Asked Questions ==

FAQ's on [Dynamic Blog Protector Homepage](http://111waystomakemoney.com/dynamic-blog-protector/)

== Screenshots ==
1. Configuration page


== Changelog ==
=1.0=
* initial release

=1.3=
*small bugs fixed